...
// Импортируем роуты из нашего роутера, но создаем тестовый роутер
import { routes } from "@/router/index";

// Создаем тестовый роутер с memory history
...
describe("Navigation Tests", () => {
  describe("Route Configuration", () => {
    it("should have warehouse route configured", () => {
      ...
      const warehouseRoute = routes.find(
        (route) => route.path === "/warehouse"
      );
    ...
    it("should have installations route configured", () => {
      const routes = router.getRoutes();
      const installationsRoute = routes.find(
        (route) => route.path === "/installations"
      );
    ...
    it("should have all main routes configured", () => {
      ...
      mainPaths.forEach((path) => {
        ...
        expect(
          ...
          `Route ${path} should require auth`
        ).toBe(true);
      });
    });
  });
  ...
  describe("Route Meta Information", () => {
    it("should have correct titles for all routes", () => {
      ...
      Object.entries(expectedTitles).forEach(([path, expectedTitle]) => {
        ...
        expect(
          route?.meta?.title,
          `Route ${path} should have correct title`
        ).toBe(expectedTitle);
      });
    });
    ...
    it("should have auth requirement for protected routes", () => {
      ...
      protectedPaths.forEach((path) => {
        ...
        expect(
          route?.meta?.requiresAuth,
          `Route ${path} should require authentication`
        ).toBe(true);
      });
    });
  });
