<template>
  <div class="object-templates">
    <div class="d-flex align-center justify-space-between mb-6">
      <div>
        <h2 class="text-h5 font-weight-bold mb-2">Шаблоны для создания объектов</h2>
        <p class="text-body-2 text-medium-emphasis">
          Управление шаблонами для создания объектов мониторинга
        </p>
      </div>
    ...
    <div v-else>
      ...
      <v-card v-else class="text-center py-8" elevation="0">
        ...
        <p class="text-body-2 text-medium-emphasis mb-4">
          Создайте первый шаблон для объектов
        </p>
...
import type { ObjectTemplate, TemplateSystem } from '@/types/settings';
...
const templates = ref<ObjectTemplate[]>([]);
...
const loadTemplates = async () => {
  loading.value = true;
  try {
    const response = await settingsService.getTemplates('object', selectedSystem.value || undefined);
    templates.value = response.templates as ObjectTemplate[];
  } catch (error) {
...
const createTemplate = () => {
  ...
  console.log('Создание шаблона объекта');
};

const editTemplate = (template: ObjectTemplate) => {
  // TODO: Реализовать редактирование шаблона
  console.log('Редактирование шаблона:', template);
};

const deleteTemplate = (template: ObjectTemplate) => {
...
const handleFileUpload = async (event: Event) => {
  ...
  try {
    ...
    importDialog.value = {
      show: true,
      template: result.template as ObjectTemplate,
