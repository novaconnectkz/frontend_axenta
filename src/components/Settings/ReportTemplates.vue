<template>
  <div class="report-templates">
    <div class="d-flex align-center justify-space-between mb-6">
      <div>
        <h2 class="text-h5 font-weight-bold mb-2">Шаблоны отчетов</h2>
        <p class="text-body-2 text-medium-emphasis">
          Управление шаблонами для создания отчетов
        </p>
      </div>
    ...
    <div v-else>
            ...
            <v-card-title class="d-flex align-center justify-space-between">
              <div class="d-flex align-center gap-3">
                <v-avatar color="info" size="40">
                  <v-icon icon="mdi-chart-line" color="white" />
                </v-avatar>
                <div>
            ...
            <v-card-text>
              ...
              <div class="d-flex align-center gap-3 mb-2">
                <div class="text-caption text-medium-emphasis">
                  ...
                  Использований: {{ template.usage_count || 0 }}
                </div>
                <v-chip
      ...
      <v-card v-else class="text-center py-8" elevation="0">
        <v-icon size="64" color="grey-lighten-1" class="mb-4">mdi-chart-line-variant</v-icon>
        <p class="text-h6 text-medium-emphasis mb-2">Нет шаблонов</p>
        <p class="text-body-2 text-medium-emphasis mb-4">
          Импортируйте первый шаблон отчетов из файла
        </p>
        <v-btn
          variant="outlined"
...
import type { ReportTemplate, TemplateSystem } from '@/types/settings';
...
const loadTemplates = async () => {
  loading.value = true;
  try {
    const response = await settingsService.getTemplates('report', selectedSystem.value || undefined);
    templates.value = response.templates as ReportTemplate[];
  } catch (error) {
...
const handleFileUpload = async (event: Event) => {
  ...
  try {
    ...
    importDialog.value = {
      show: true,
      template: result.template as ReportTemplate,
