...
class AxentaAutoRefreshService {
  ...
  private async refreshDashboardData() {
    try {
      // Очищаем кеш перед обновлением для получения свежих данных
      dashboardService.clearStatsCache();
      
      // Обновляем статистику дашборда с принудительным обновлением
      await dashboardService.getStats(true);
